...
let lluviaCorazonesActiva = false,
  contadorToques = 0;
let objetosMensaje = [];
...
const URL_TEXTURA_SATURNO =
  "https://bcodestorague.anteroteobaldob.workers.dev/share/anteroteobaldob_gmail_com/TEXTURAS/textura_sat.jpg";

const textoAnillo =
  "TE AMO CON TODO MI CORAZÓN Y ALMA PARA SIEMPRE MI AMOR ETERNO";
...
const crearSaturno = () => {
  ...
  const materialTemporal = new THREE.MeshPhongMaterial({
    color: 0xddaa77,
    shininess: 10,
  });
...
const crearAnilloTexto = () => {
  ...
  const anilloInterno = new THREE.Mesh(
    geometriaAnilloInterno,
    materialAnilloInterno
  );
  ...
  const anilloExterno = new THREE.Mesh(
    geometriaAnilloExterno,
    materialAnilloExterno
  );
...
const crearCampoEstrellas = () => {
  ...
  geometriaEstrellas.setAttribute(
    "position",
    new THREE.Float32BufferAttribute(verticesEstrellas, 3)
  );
  const campoEstrellas = new THREE.Points(
    geometriaEstrellas,
    materialEstrellas
  );
...
const crearTextoRespaldo = (mensaje) => {
...
const configurarEventos = () => {
  ...
  document
    .getElementById("alternar-audio")
    .addEventListener("click", alternarAudio);

  window.addEventListener("resize", manejarRedimensionamiento);
  window.addEventListener("wheel", manejarDesplazamiento);
};

const manejarMovimientoCamara = () => {
...
const reproducirAudio = () => {
  ...
  if (audioHabilitado) {
    audio
      ...
      .catch((e) => {
        console.log(
          "Audio requiere interacción del usuario primero para reproducirse."
        );
      });
  }
};
...
const crearExplosionTexto = (e) => {
  ...
  const textoAleatorio =
    textosAmor[Math.floor(Math.random() * textosAmor.length)];
  ...
  explosionTexto.style.animation =
    "animacionExplosionTexto 1.5s ease-out forwards";
...
const animar = () => {
  ...
  objetosMensaje.forEach((obj, i) => {
    obj.malla.position.y =
      obj.alturaOriginal + Math.sin(tiempo * obj.velocidad + i) * 1.5;
