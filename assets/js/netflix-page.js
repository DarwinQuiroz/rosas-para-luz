...
let rutaAssets = arrUrl[2].includes("localhost")
  ? "../assets/"
  : "../rosas-para-luz/assets/";
...
class MusicPlayer {
  constructor(
    ...
    this.songInfo = {
      ...
      cover: "disco.png",
      audio: `${rutaAssets}sound/die-for-you.mp3`,
    };
  ...
  initializeSharedAudio() {
    ...
    this.audioPlayer.addEventListener("timeupdate", () => {
      if (
        ...
        this.player.closest("#music-player-section")?.style.display === "block"
      ) {
  ...
  checkAudioAvailability() {
    ...
    fetch(this.songInfo.audio, { method: "HEAD" })
      .then((response) => {
        if (!response.ok) {
          console.error(
            `Audio file not accessible: ${this.songInfo.audio}, Status: ${response.status}`,
          );
        } else {
      ...
      .catch((error) => {
        console.error(
          `Error checking audio file: ${this.songInfo.audio}`,
          error,
        );
      });
  }
  ...
  attachEventListeners() {
    ...
    const volumeSliderVertical = this.volumeSliderContainer.querySelector(
      ".volume-slider-vertical",
    );
    if (volumeSliderVertical) {
      volumeSliderVertical.addEventListener("click", (e) =>
        this.setVolumeFromVerticalBar(e),
      );
    }
  ...
  toggleFavorite() {
    ...
    if (window.profilePlayer && this !== window.profilePlayer) {
      ...
      window.profilePlayer.favoriteBtn.textContent = this.isFavorite
        ? "❤"
        : "♡";
      window.profilePlayer.favoriteBtn.classList.toggle(
        "active",
        this.isFavorite,
      );
    } else if (window.musicPlayer && this !== window.musicPlayer) {
      ...
      window.musicPlayer.favoriteBtn.classList.toggle(
        "active",
        this.isFavorite,
      );
    }
  ...
  updateDurationDisplay() {
    if (
      ...
      !isNaN(this.audioPlayer.duration) &&
...
document.addEventListener("DOMContentLoaded", function () {
  window.profilePlayer = new MusicPlayer(
    "player",
    "audioPlayer",
    "playBtn",
    "favoriteBtn",
  ...
  const episodeModalDescription = document.getElementById(
  ...
  function updateSection(section) {
    ...
    if (section === "inicio") {
      ...
      yearDuration.innerHTML = "13+ 2025 • 1 episodios • Romance • Drama";
      seriesDescription.innerHTML =
        "Eres el tesoro más valioso que tengo, prometo cuidarte y estar contigo en las buenas y en las malas. ♥";
